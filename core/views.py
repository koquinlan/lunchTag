...
def homepage(request):
    if request.user.is_authenticated:
        ...
        return render(
            ...
            context={
                "pairing": profile.tag_pairing,
                "pair_profile": pair_profile,
                "pairing2": profile.tag_pairing2,
                "pair_profile2": pair_profile2,
            },
        )
...
def help_request(request):
    return render(request=request, template_name="help/help.html")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                messages.error(
                    request,
                    "Unsuccessful registration. It's likely a user with this First and Last name already exists. Try logging in or contact admin for help if needed.",
                )
            ...
            messages.success(
                request,
                f"Registration successful, You are now logged in as {newUser.first_name} {newUser.last_name}.",
            )
        ...
        messages.error(
            request,
            "Unsuccessful registration. Invalid information, likely an invalid password. Password must be at least 8 characters, contain at least 1 letter, and not be extremely common (ie. password123) or too similar to your name/email!",
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                messages.success(
                    request,
                    f"You are now logged in as {user.first_name} {user.last_name}.",
                )
...
def account_request(request):
    ...
    return render(
        ...
        "accounts/account.html",
        {
            ...
            "allusers": User.objects.order_by("first_name"),
            "userstrikes": profile.strikes.order_by("first_name"),
        },
    )
...
def strike_request(request, userID):
    ...
    messages.error(
        request,
        "Strike may have failed, please contact admin with contact above to confirm or try again",
    )
    return redirect(to="core:account")
...
def remove_crush_request(request):
    ...
    messages.error(
        request, "Removing crush may have failed, please contact admin to confirm"
    )
    return redirect(to="core:account")
...
def remove_strike_request(request, userID):
    ...
    messages.error(
        request,
        "Removing this strike may have failed, please contact admin with contact above to confirm or try again",
    )
    return redirect(to="core:account")
...
def toggle_active_request(request):
    ...
    if profile.active:
        messages.success(
            request, "You have opted back in and will be included in the next pairing!"
        )
    else:
        messages.warning(
            request,
            "You have opted out and will NOT be included until you opt back in!",
        )
    return redirect(to="core:account")
...
def edit_pairs_request(request):
    ...
    return render(request, "pairings/edit_pairings.html", {"allpairings": allpairings})
...
def create_pairings_request(request):
    ...
    if code == -1:
        messages.error(
            request,
            "For whatever reason this pairing tried to match someone with their strike. Could try again but probably need to contact Kyle. Sorry...",
        )
    elif code == 2:
...
def push_pairings_request(request):
    ...
    if code == -1:
        ...
        messages.error(
            request,
            "This error code is extra bad!! Pairings were pushed but there may have been a pair that overrode a strike. Contact Kyle or remake and try again.",
        )
    elif code == 1:
