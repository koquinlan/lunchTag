...
def make_pairings(request):
    ...
    for pair in pairings:
        ...
        if (user2 in profile1.strikes.all()) or (user1 in profile2.strikes.all()):
            print("Two struck users were in the list of pairings. Please check and redo!")
            return -1
    ...
    if (len(Profile.objects.filter(active=True)) % 2) == 1:
        for profile3 in Profile.objects.filter(active=True):
            ...
            if profile3.has_tag_pairing == False:
                for pair in Pairing.objects.all():
                    ...
                    if((user1 not in profile2.strikes.all()) and (user1 not in profile3.strikes.all()) and (user2 not in profile1.strikes.all()) and (user2 not in profile3.strikes.all()) and (user3 not in profile1.strikes.all()) and (user3 not in profile2.strikes.all())):
                ...
                messages.error(request, "For some reason we were unable to find a valid pairing for " + profile3.user.first_name + " " + profile3.user.last_name + ". They may have no valid pairings... Contact Kyle or try again.")
                return -2
...
def push_pairings(pairings, request):
    ...
    for pair in pairings:
        if pair.user3 == None:
            ...
            if((user1 in profile2.strikes.all()) or (user1 in profile3.strikes.all()) or (user2 in profile1.strikes.all()) or (user2 in profile3.strikes.all()) or (user3 in profile1.strikes.all()) or (user3 in profile2.strikes.all())):
                print("Two struck users were in the list of pairings. Please check and redo!")
                return -1
    ...
    for profile in Profile.objects.filter(active=True):
        ...
        if profile.has_tag_pairing == False:
            messages.error(request, "For some reason we were unable to find a valid pairing for " + profile.user.first_name + " " + profile.user.last_name + " and they were left unpaired. :(")
            return -4
...
def create_edges(user1, user2, active=True, weight=100.0):
    ...
    for edge in user1.user1.all():
        if edge.user2 == user2:
            print( f"Edge btwn {user1.username} and {user2.username} already exists with weight {edge.weight}. Active = {edge.active}")
            return
